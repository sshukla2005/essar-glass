...
const CustomerForm = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => isEdit ? customerApi.update(id, data) : customerApi.create(data),
    onSuccess: () => {
  ...
  const handleSave = async (andNew = false) => {
    try {
      ...
      if (andNew) { form.resetFields(); navigate('/masters/customers/new') }
      else navigate('/masters/customers')
    } catch (_) {}
  }
      ...
      children: (
        <>
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item name="name" label="Customer Name" rules={[{ required: true, message: 'Customer name is required' }]}>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              ...
              <Form.Item name="email" label="Email"><Input placeholder="customer@example.com" type="email" /></Form.Item>
    ...
    },
    {
      ...
      children: (
        <>
          ...
          <Row gutter={16}>
            <Col span={8}><Form.Item name="salesperson" label="Salesperson"><Input placeholder="Salesperson name" /></Form.Item></Col>
            <Col span={8}><Form.Item name="sales_team" label="Sales Team"><Input placeholder="Sales team name" /></Form.Item></Col>
            <Col span={8}>
          ...
          <Row gutter={16}>
            <Col span={8}><Form.Item name="pricelist" label="Pricelist"><Input placeholder="Default pricelist" /></Form.Item></Col>
            <Col span={8}>
    ...
    },
    {
      ...
      children: (
        <Form.Item name="internal_notes" label="Notes">
          <TextArea rows={6} placeholder="Add any internal notes about this customer..." />
        </Form.Item>
      ),
    },
  ]
  ...
  return (
    <MasterForm
      ...
      breadcrumbs={[{ label: 'Masters' }, { label: 'Customers', path: '/masters/customers' }, { label: isEdit ? 'Edit' : 'New' }]}
      onSave={() => handleSave(false)}
      onSaveNew={() => handleSave(true)}
      onDiscard={() => navigate('/masters/customers')}
    >
